...
describe('<Event /> component', () => {
  ...
  test('renders event start time', () => {
    const startTime = new Date(allEvents[0].start.dateTime).toLocaleTimeString(); // Convert start time to a readable format
  ...
  test('details section is shown when "Show Details" button is clicked', () => {
    ...
    const details = EventComponent.getByText('Details section content goes here');
    expect(details).toBeInTheDocument();
  });
  ...
  test('details section is hidden when "Hide Details" button is clicked', () => {
    ...
    expect(EventComponent.queryByText('Details section content goes here')).not.toBeInTheDocument();
  });
